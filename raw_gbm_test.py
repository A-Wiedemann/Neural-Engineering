...
train = pd.DataFrame(columns=['subject','session','feedback_num','start_pos'] + ['CP2_' + s for s in map(str,range(261))],index=range(5440))
counter = 0
...
for i in train_subs:
    for j in range(1,6):
        ...
        counter2 = 0
        for k in fb.index:
                temp2 = temp.loc[int(k):int(k)+260,'CP2']
                temp2.index = ['CP2_' + s for s in map(str,range(261))]
                train.loc[counter,['CP2_' + s for s in map(str,range(261))]] = temp2
...
train.to_csv('train_CP2.csv',ignore_index=True)

test = pd.DataFrame(columns=['subject','session','feedback_num','start_pos'] + ['CP2_' + s for s in map(str,range(261))],index=range(3400))
...
for i in test_subs:
    for j in range(1,6):
        ...
        for k in fb.index:
                temp2 = temp.loc[int(k):int(k)+260,'CP2']
                temp2.index = ['CP2_' + s for s in map(str,range(261))]
                test.loc[counter,['CP2_' + s for s in map(str,range(261))]] = temp2
...
test.to_csv('test_CP2.csv',ignore_index=True)
