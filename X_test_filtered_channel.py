...
for f in list_of_train_files:
    ...
    for feedback_event in events:
        ...
        if first_add ==0:
            X_test_filtered_channel = feature_vector.transpose()
            first_add=1
        else:
            X_test_filtered_channel = np.vstack((X_test_filtered_channel,feature_vector.transpose()))
    #X_train_average.append(X_buffer,axis=0)
    print('X_test_filtered_channel: ', np.shape(X_test_filtered_channel))
    it_file+=1

#print('X_train_average: ', np.shape(X_train_average))

np.savetxt('X_test_filtered_channel.csv', X_test_filtered_channel, delimiter=",")
